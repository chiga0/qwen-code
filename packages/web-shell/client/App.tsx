...
export function App({
  ...
  const loadMcpManagerMessage = useCallback(async () => {
      ...
      (status?.servers ?? []).map(async (server) => {
        await Promise.all([
          ...
          server.resourceCount
            ? workspaceActions
                .loadMcpResources(server.name)
                .then((resources) => {
                  resourcesByServer[server.name] = resources;
                })
                .catch(() => undefined)
            : Promise.resolve(),
        ]);
    ...
    const message: SerializedMcpStatusMessage = {
      status,
      toolsByServer,
      resourcesByServer,
  ...
  useEffect(() => {
    ...
    const activeWorkspace = workspace.capabilities?.workspaces?.find(
      (entry) => entry.cwd === connection.workspaceCwd,
    );
    if (connection.sessionId && !activeWorkspace) return;
    const workspaceId =
    ...
    (
      ...
      if (text.startsWith('/')) {
        ...
        if (match) {
          ...
          if (cmd === 'mcp') {
            openPanel('mcp');
            void loadMcpManagerMessage().catch((error: unknown) => {
          ...
          if (cmd === 'skills') {
            ...
            if (skillArg === 'detail' || skillArg === 'details') {
              openPanel('skills');
            } else if (skillArg) {
              if (promptBlocked) {
                return enqueuePrompt(
                      ...
                      {activePanel === 'settings' ? (
                        ...
                        mcpDialogMessage ? (
                          ...
                          <div className="flex min-h-48 items-center justify-center gap-2 text-sm text-muted-foreground">
                            <Spinner />
                            <span>{t('mcp.loadingStatus')}</span>
                          </div>
                        )
                      ) : activePanel === 'extensions' ? (
                        <ExtensionsManagerPage onClose={closePanel} />
                      ) : activePanel === 'tools' ? (
                        <ToolsManagerPage onClose={closePanel} />
                      ) : activePanel === 'skills' ? (
                        <SkillsManagerPage
                          onClose={closePanel}
                          onUseSkill={(name) => {
                      ...
                      >
                        ...
                        <div className={styles.composer}>
                          ...
                          {escapeHintVisible && streamingState === 'idle' && (
                            <div
                              className={styles.escClearStatus}
                              role="status"
                            >
                          ...
                          <ChatEditor
                            ...
                            workspaces={
                              (workspace.capabilities?.workspaces?.length ??
                                0) > 1
                                ? (
                                    workspace.capabilities?.workspaces ?? []
                                  ).map((entry) => ({
